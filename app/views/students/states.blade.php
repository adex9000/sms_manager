        ...
        <div class="col-lg-12">
            <div class="panel panel-default">
                <div class="panel-heading">
                    <h3 class="panel-title">Students: State of Origin</h3>
                </div>
                <div class="panel-body">
                    <div class="row">
                        ...
                        <div class="row">
                            <div class="col-lg-6">
                                <form action="{{ URL::route('search_states') }}" class="form-inline" role="form" method="get">
                                    <div class="form-group">
                                        <label for="states">State of Origin</label>
                                        {{ Form::select('search',$states,$search,['class'=>'form-control','id'=>'states']) }}
                                    </div>
                                    ...
                                    @if($results && count($results) > 0)
                                        @foreach($results as $student)
                                            <tr>
                                                ...
                                                <td>{{ Utilities::expandState($student->stateid) }}</td>
                                                <td>
                                                    <form action="{{ URL::route('send_sms_message') }}" role="form" method="post">
                                                        {{ Form::token() }}
                                        ...
                                        @if($results->getTotal() > 30)
                                            <tr>
                                                <td colspan="9">{{ $results->appends(['search' => $search])->links()  }}</td>
                                            </tr>
                                        @endif
